	...
	if line[0:3] == "CHR":
		...
		if int(start) - start_current > min_dis:
			...
			if len(index_Non_nan_genotypes) > 10:
				...
				if tem_total_allele1 < len(tem_filterd_genotypes):
						...
						if line2[0:3] == "CHR":
							...
							if (int(start2) > int(start)) and (int(start2) - start2_current > min_dis):
								...
								if len(total_non_na) > 10:
									...
									p1=sum(alleles_genotypes)/total
									q1=sum(alleles_genotypes2)/total
									# final check that both varint sites are polimorphic:
									if (q1 < 1) and (p1 < 1):
