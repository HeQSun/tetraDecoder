...
for line in open(file, "r"):
	...
	if line[0:3] == "CHR":
		output_popParameters.write(f'chr\twin\tsum_pi\tSS\tNvar\tmean_pi\tW_theta\tTaj_D\n')
	else:
		...
		if Nsamples > 1:
			...
			if win!=current_win:
				if current_win == 0:
					...
					output_popParameters.write(f'{chr}\t{current_win}\t{sum_pi}\t{total_SS}\t{NVar}\t{sum_pi/winSize}\t{total_W_theta}\t{Taj_D}\n')
					current_win = win
					sum_pi = pi
					total_SS = SS
					W_theta = w_theta_local
