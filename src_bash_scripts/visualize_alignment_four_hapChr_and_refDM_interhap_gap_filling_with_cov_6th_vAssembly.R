## this is setting up for visualizing alignment of chr to DM ref chr, with coverage analysis of chrs of 6th version of assembly
library(scales)
##
...
}else
{
  ...
  ####for (lg in c(paste("chr0", 1, sep="")))
  {
    ...
    cov <- read.table(paste("/netscratch/dep_mercier/grp_schneeberger/projects/Potato_multipleCultivars/s2_10_Cultivars_PacBio_HiFi/a8_make_full_genome_tmp/","v6th","_tmp_full_", cultivar,"/shallow_DNB_aln/cnv_winsize50000_step50000_hq.txt", sep=""))
  # example input
  # cultivar="A"
  # lg="chr01"
  # hapstr="1,2,3,4"
  ...
  # out_prefix="A_chr01"
    ...
    for(hapi in haps)
    {
      ...
      # aln
      if(strand=="_gap_filling")
      {
        alignmraw2       <- read.table(paste(path, "/build_new_seq_from_old_", hapi, "/zcorrection_gap_corr_","6th","_refDM/", aln_file, sep=""))
      }
