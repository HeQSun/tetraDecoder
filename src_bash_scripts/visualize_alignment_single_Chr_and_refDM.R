...
if(length(args) < 5) {
  ...
  cat("   USAGE: Rscript visualize_alignment_single_Chr_and_refDM.R cultivar lg_hapi align_file out_path out_prefix\n\n")
}else
{
  ## check args
  cultivar   = args[[1]]
  ...
  par(mfrow=c(1,1), mai = c(0.5, 0.1, 0.5, 0.3)); # margin: bottom, left, top, right
  #
  ####################################################################################################################
  ################################# double monoploid ref: totalN_12047830_of_860168943 #################################
  if(1)
  {
    alignmraw2       <- read.table(paste(path, "/", aln_file, sep=""))
    ...
    title(main = paste("DM-Ref v.s. ", cultivar, ": ", lg, sep=""), cex.main=0.8, font.main=1)
    # legend
    legend(90000000, -0.2,
           pch    = c(15,15),
