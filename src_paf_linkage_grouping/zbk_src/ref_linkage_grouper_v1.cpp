...
int main(int argc, char* argv[])
{
    ...
    map<string, unsigned long> ctg_size;
    map<string, map<string, map<unsigned long, unsigned long> > > aln; 
    if( !read_paf(paf_file, &ctg_size, &aln) )
    {
              ...
              map<string, map<string, map<unsigned long, unsigned long> > >* aln)
{
    ...
    */    
    ...
    while(ifp.good())
    {
        ...
        //   
        if(this_check_on)
        cout << "   check: ctg " << this_ctg_id << endl;
        // insert new info
            ...
            }else
            {
                ...
                while(pitr != pitr_end)
                {
                    ...
                    if(ex_sta<=this_ref_sta && this_ref_sta<=ex_end && ex_end<=this_ref_end)
                    {
                        ...
                        */
                        if(this_check_on)
                        cout << "   check: overlapping1 at "<< this_ref_id 
                             << " "                         << ex_sta
                             << " "                         << ex_end
                             << " vs "                      << this_ref_sta
                             << " "                         << this_ref_end
                    ...
                    if(ex_sta<=this_ref_sta && this_ref_end<=ex_end)
                    {
                        ...
                        */
                        if(this_check_on)
                        cout << "   check: overlapping2 at "<< this_ref_id
                             << " "                         << ex_sta
                             << " "                         << ex_end
                             << " vs "                      << this_ref_sta
                             << " "                         << this_ref_end
                    ...
                    if(this_ref_sta<=ex_sta && ex_sta<=this_ref_end && this_ref_end<=ex_end)
                    {
                        ...
                        */   
                        if(this_check_on)                 
                        cout << "   check: overlapping3 at "<< this_ref_id 
                             << " "                         << ex_sta
                             << " "                         << ex_end
                             << " vs "                      << this_ref_sta
                             << " "                         << this_ref_end
                    ...
                    if(this_ref_sta<=ex_sta && ex_sta<=this_ref_end && this_ref_sta<=ex_end && ex_end<=this_ref_end)
                    {
                        ...
                        */
                        if(this_check_on)
                        cout << "   check: overlapping4 at "<< this_ref_id 
                             << " "                         << ex_sta
                             << " "                         << ex_end
                             << " vs "                      << this_ref_sta
                             << " "                         << this_ref_end
