...
int main(int argc, char* argv[])
{
    ...
    if(argc < 4)
    {
        ...
        printf("Usage: var_summer common-path output_list label\n\n");
        cout << "Info: common-path+label => path/to/shore_consensus_het_local_mkdup/ConsensusAnalysis/quality_variant_iQ32_simple.txt" << endl;
    ...
    string markerfiles = (string)argv[1];
    vector<string> mfinfo = split_string(markerfiles, ',');
    if(mfinfo.size() < 2)
    {
        cout << "   Info: only one list of variations provided. No need to continue. Stopped. " << endl;
        return false;
    }
    int output_list = atoi(argv[2]);
    string output_label = (string)argv[3];
    //
    ...
    while(vitr != vitr_end)
    {
        ...
        cout << "   Info: checking var file " << this_var_file << endl;
