...
if(length(args) < 3) {
  cat(length(args))
  cat("\n   Info: find potentially missing regions during assembly (ref-coordinate based).\n")
  cat("   USAGE: Rscript qry_ref_aln_cov_analysis_func 1.sample 2.lg 3.real_hapi 4.path\n\n")
}else
{
  ## check args
  sample    = args[[1]]
  lg        = args[[2]]
  real_hapi = args[[3]]  
  path      = paste("/netscratch/dep_mercier/grp_schneeberger/projects/Potato_multipleCultivars/s2_10_Cultivars_PacBio_HiFi/a6_lg_wise_scaffolding/scaf_",sample,"/s_read_realign_",lg,"/zcorrection_four_hap_juicebox_DM_strand_interhap_correction/zcorrection_",real_hapi,"_phase_corr_4th_refDM/", sep="")
  setwd(path)
  #
  # sample_list="A"
  # lg_list="chr01"
