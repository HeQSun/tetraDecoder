...
unsigned long min_hapctg_size = 500000; // min sizes of contigs to build backbone of chr-groups!!!!!!!!!!!!!!!!!!!!!!!!!
...
bool merge_clusters(map<string, double>                    cor_any,
                  ...
                  map<int, map<string, int> >*             cluster_vertex_upd,   
                  map<int, unsigned long>*                 cluster_vertex_size_upd);
int allelic_check(map<string, int>                         smallest_cluster_vertex,
...
bool analyze_hic_matrix(map<string, int>                   ctg2_hic_group,
                  map<int, map<string, int> >              hic_group_2ctg,
                  string                                   out_prefix); // involve global win_marker and hic_matrix
//
int main(int argc, char* argv[])
{
    ...
    while (std::getline(std::cin, read_a)) 
    {
        ...
        unsigned long scale_end_region = 5;
        unsigned long left_sta_a_ctg = 1;
        unsigned long left_end_a_ctg = min_hapctg_size*scale_end_region;
        ...
        if(length_a_ctg > min_hapctg_size*scale_end_region)
        {
            right_sta_a_ctg = length_a_ctg - min_hapctg_size*scale_end_region + 1;
        }
        ...
        unsigned long left_end_b_ctg = min_hapctg_size*scale_end_region;
        ...
        if(length_b_ctg > min_hapctg_size*scale_end_region)
        {
            right_sta_b_ctg = length_b_ctg - min_hapctg_size*scale_end_region + 1;
        } 
 ...
 // string new_hic_group_file = tmpfolder_s6 + "/s7_haplotig_hic_contact_matrix_subclusters_hap_extended.dot\0";
    ...
    if( !analyze_hic_matrix(ctg2_hic_group, hic_group_2ctg, out_prefix) )
    {
                             ...
                             map<int, map<string, int> >*    hic_group_2ctg)
{
    /*
        ...
        map<string, ENDREGION>         ctg_end_region           // <ctg_id, {ctg_left_sta/end, ctg_right_sta/end} >        
        map<int, int>                  cluster_id_upd;          // updated <clusterid_old, clusterid_new>    
        map<int, vector<int> >         merging_record;          // <clusterid_old, <merged-subcluster-old-ids> >                 
        map<int, map<string, double> > cluster_edge_upd;        // <clusterid_old, <ctg1-ctg2, hic.contact.value > >
        map<int, map<string, int> >    cluster_vertex_upd;      // <clusterid_old, <ctg_id, 1> >  
        map<int, unsigned long>        cluster_vertex_size_upd; // <clusterid_old, total_ctg_length >   
        map<string, int>               ctg2_hic_group           //                 <ctg_id, clusterid_old>
        map<int, map<string, int> >    hic_group_2ctg           // <clusterid_old, <ctg_id, clusterid_new> > 
        # global
        map<string, CONTACT>           ctg_cross_link_long_hap2 // < ctg1-ctg2, {LL, LR, RL, RR} >
        map<string, CONTACT>           ctg_cross_link_other2    // < ctg1-ctg2, {LL, LR, RL, RR} >      
        map<string, map<unsigned long, NODE> > hap_win_marker   // <ctg_id, <win_sta, {win_end, hap/dip/trip/tetrap/rep, size_ctg, size_window, ...}> >
        map<string, map<unsigned long, NODE> > other_win_marker // includes short or non-pure haplotigs and non-haplotigs
    ...
    */
                ...
                while(citr != citr_end)
                {
                    ...
                    {                        
                        ...
                        */
                        ...
                        ctg1_end_size    = ctg_end_region[ this_ctg  ].ctg_left_end - ctg_end_region[ this_ctg  ].ctg_left_sta + 1;
                        ctg2_end_size    = ctg_end_region[ known_ctg ].ctg_left_end - ctg_end_region[ known_ctg ].ctg_left_sta + 1;                  
                        if(tmp_contact.LR > this_cnt)
                        {
                            this_cnt     = tmp_contact.LR;
                            contact_type = "LR"; 
                            ctg1_end_size    = ctg_end_region[ this_ctg  ].ctg_left_end  - ctg_end_region[ this_ctg  ].ctg_left_sta  + 1;
                            ctg2_end_size    = ctg_end_region[ known_ctg ].ctg_right_end - ctg_end_region[ known_ctg ].ctg_right_sta + 1; 
                        }
                        if(tmp_contact.RL > this_cnt)
                        {
                            this_cnt     = tmp_contact.RL;
                            contact_type = "RL";    
                            ctg1_end_size    = ctg_end_region[ this_ctg  ].ctg_right_end - ctg_end_region[ this_ctg  ].ctg_right_sta + 1;
                            ctg2_end_size    = ctg_end_region[ known_ctg ].ctg_left_end  - ctg_end_region[ known_ctg ].ctg_left_sta  + 1;                           
                        }
    ...
    /////////////////////////////// Part II: integrate non-pure haplotigs and non-haplotigs //////////////////////////// TODO
    ...
    while(cluster_after_merge > N_cluster)
    {
        ...
        while(svitr != svitr_end)
        {
            ...
            while(vclitr != vclitr_end)
            {
                ...
                int allelic_cnt = allelic_check(smallest_cluster_vertex, larger_cluster_vertex); 
                if(allelic_cnt > 2)
                {
