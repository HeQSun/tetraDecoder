                       ...
                       map<string, ENDREGION>     ctg_end_region)
{
    ...
    map<string, double> cor_all;           // <ctg1-ctg2, hic.contact.value >    
    ...
    while(litr != litr_end)
    {
        ...
        if(this_label.compare("hap")==0 && normalized_reads > min_contact) // another level of haplotig marker selection
        {
            ...
            {
                cor_all.insert(std::pair<string, double>(edge1, normalized_reads) );
            }else
            {
                // should never happen
    ...
    // core function: clustering of haplotigs
    if(this_label.compare("hap")==0)
    {
        ...
        string subclusterinfo = tmpfolder + "/s5_haplotig_hic_contact_matrix_subclusters_raw.dot\0"; // 
        ...
        subcluster_ofp << "/* Here are the raw subclusters of contigs */" << endl;
