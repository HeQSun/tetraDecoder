                      ...
                      map<string, map<unsigned long, map<unsigned long, WHIC> > > hic_matrix)
{
    ...
    if(!refine_hb_window_marker_intra_ctg(ctg_order, hbmarker, ctg2lg, "1st", out_folder))
    {
        return false;
    }
    ...
    if(!refine_hb_window_marker_intra_ctg(ctg_order, hbmarker, ctg2lg, "2nd", out_folder))
    {
        return 1;
    }
