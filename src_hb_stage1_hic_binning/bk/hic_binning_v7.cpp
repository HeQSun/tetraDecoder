...
map<string, string> allelic_map;       // allelic relationship between contigs <ctg1#ctg2, "0"> 
//
...
int main(int argc, char* argv[])
{
    if(argc < 8)
    {
        ...
        cout << "(compiled on " << buildString << ")" << endl
             ...
             << " |"
             << " hic_binning - contact_cutoff target_contig_size.txt window_markers.txt min_hapctg_size allelic.table.txt out_prefix_str"
             << endl 
             << endl
    ...
    string out_prefix = (string)argv[7];    
    ...
    while (std::getline(std::cin, read_a)) 
    {
        ...
        // if the contigs show allelic relationship, skip the links..
...
int allelic_check(map<string, int> smallest_cluster_vertex, 
                  map<string, int> larger_cluster_vertex)
{
    /* check if there are contigs in small list having allelic relationship with those in large list, with 
