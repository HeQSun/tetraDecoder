                      ...
                      map<string, unsigned long> target_contig_size)
{
    ...
    cout << "         refined marker collected in "              << new_hbmarker_file << endl;
    cout << "   Info: 1st-refining hic_binning grouped window marker done."           << endl;    
    cout << endl;
    // s2: inter-grp refinement  
    cout << "   Info: reading hic_binning grouped window marker info for 2nd inter-grp refinement.. " << endl;
    ctg_order.clear();// <ctg_id> keep input order of contigs
    hbmarker.clear(); // <ctg_id, <win_sta, {win-end, type=hap/dip/..., wlg=<lgs>, 1:12 } > >
    ctg2lg.clear();   // <ctg_id, <lg_id=1:4, 1:12> >
    homLGs.clear();   // <lg_id=1:4,  homLG_id=1:12>
    ...
    cout << "   Info: reading hic_binning grouped window marker info for 2nd inter-grp refinement done. " << endl;
    cout << endl;
    string hic_matrix_file = out_folder + "/zadd_hic_matrix.txt\0"; 
    if(!refine_hb_window_marker_inter_grp(ctg_order, hbmarker, ctg2lg, hic_matrix_file, out_folder))
    {
        return 1;
    }
    // s3: intra-ctg refinement
            ...
            for (int oi = 0; oi < out_num; oi ++)
            {
                ...
                {                
                    ofp  << this_ctg 
                       ...
                       //<< "\t"        << raw_line_part
                         << endl;   
                    hbmarker_grp_hic[this_ctg][this_win_sta].lg_hic_sum.insert(std::pair<string, double>(tmp_marker2.lg, 111 ));                         
                }else            
    ...
    */
    ...
    while(citr != citr_end)
    {
        ...
        while(hitr != hitr_end)
        {
            ...
            double avg_to_grp_hic = link_lg_hic[ linked_lg ]*1.0;
            cout << "         | average hic-link score with LG-"<< linked_lg << ": "<< avg_to_grp_hic / link_lg_cnt[ linked_lg ] << endl;            
            //
            if(final_lgs.size() == 0)
            {
                final_lgs.push_back(    linked_lg   );
                final_hic.push_back( avg_to_grp_hic );    
                //
            ...
            }else
            {
                ...
                for(int ii = 0; ii < final_lgs.size(); ii ++)
                {
                    if( final_hic[ii] <= avg_to_grp_hic )
                    {
                        final_lgs.insert(tmp_lg_itr, linked_lg);
                        final_hic.insert(tmp_lg_hic_itr, avg_to_grp_hic);
                        //
