...
int main(int argc, char* argv[])
{
    ...
    */
    ...
    // tetraplotig goes to all four; triplotig goes to the opposite three haps of allelic-and-assigned haplotigs
    // clean hic matrix; if ctg1 is allelic to ctg2, ctg1 and ctg3 in backbone clusters, the link between ctg2 and ctg3 should be removed!
    
    // how do I make use of hic links among non-haplotigs? E.g, a diplotig highly linked to a triplotig, indicating they have at least one hap in common? 
    ...
    if( !analyze_hic_matrix(ctg2_hic_group, hic_group_2ctg, ctg_allelic_group, out_prefix, tmpfolder_s6) )
    {
    ...
    if(!group_refinement( raw_hb_grouping_file, tmpfolder_s6, hic_matrix, target_contig_size) )
    {
            ...
            for(int ci = 0; ci < 2; ci ++)
            {
                ...
                if( (*ctg2_hic_group).find(this_ctg) == (*ctg2_hic_group).end() )
                {
                    ...
                    while(lvitr != lvitr_end)
                    {
                        ...
                        if( allelic_cnt[ this_old_group_id ] > 5 && allelic_ctg_size_overlap_ratio[ this_old_group_id ]> 0.01) // TODO need an option on threshold of "5"
                        {
                    ...
                    // if(best_max_link_val > 3.5) // CAUTION on setting here! TODO setting up option                
    ...
    while(cluster_vertex.size() > 4)
    {
        ...
        while(gitr1 != gitr_end)
        {
            ...
            while(gitr2 != gitr_end)
            {
                ...
                // TODO: set on options on threshold below '5' and '0.01'                     
                // if(allelic_ctg_cnt_group12 <= 5 || allelic_ctg_size_overlap_ratio<0.01) 
                if(allelic_ctg_size_overlap_ratio<0.01)                 
                {
                    cout <<                "." << endl;
                }else
                {
                ...
                */
                if(grp_contact[group1_id][group2_id] / individual_cnts > to_merge_hic_max && 
                   (allelic_ctg_size_overlap_ratio<0.01) 
                  )
                {
        ...
        {
            cout << "   Warning: cluster number cannot be merged into the expected value of 4." << endl;
            break;
        }
    }
    //
                        ...
                        string                      out_folder)
{
    /* Function: for each marker not phased, assign it to 1~4 initial backbone-clusters based on marker type 
          ...
          map<int, map<string, int> >............................ hic_group_2ctg: < clusterid_old, <ctg_id, clusterid_new> >           
          map<string, map<int, map<string, unsigned long> > > ... ctg_allelic_group: <ctg, <allelic_grp_id, <allelic_ctg, allelic_ctg_size > > >
          string .....................................................out_prefix: lable linkage group, chr1, chr2, ....
          map<string, map<unsigned long, NODE> >..................... win_marker: < ctg_id, <win_sta, {win_end, type, win_size, ctg_size, grp_link_val, grp_link_ctg, grp_link_val_sum, grp_link_val_cnt, grp_link_val_size} > >
          map<string, map<unsigned long, map<unsigned long, WHIC> > > hic_matrix: < ctg1-ctg2, <sta1, <sta2, {end1,typ1,end2,typ2,hic-cnt} > > >          
             WHIC
   	       unsigned long     end1; // marker-window end of ctg1
    	       string            typ1; // type of window of ctg1: hap/dip/...
    ...
    */
    ...
    while(mitr != mitr_end)
    {
        ...
        while(hitr != hitr_end)
        {
            ...
            allelic_ctg_size_overlap_ratio = allelic_ctg_size_overlap * 1.0 / (allelic_ctg_size + 1);
        ...
        if( ctg2_hic_group.find(this_ctg) == ctg2_hic_group.end() )
        {
            ...
            while(witr != witr_end)
            {
                ...
                while(hitr != hitr_end)
                {
                    ...
                    // double linked_val = tmp_node.grp_link_val_sum[linked_lg] / tmp_node.grp_link_val_cnt[linked_lg] / tmp_node.grp_link_val_size[linked_lg]; 
                ...
                cout << "      * ----" << endl;                
                // cleaning candidate group with allelic groups 
                int num_to_keep = -1;
                ...
                for(int ii = 0; ii < wind_lgs.size(); ii ++)
                {
                    cout << "      * candi-grp ordering after cleaning: LG-" << wind_lgs[ii] << " with score " << wind_hic[ii] << endl;
                    //
                    if(ii < num_to_keep)
                    {
                        int    tmp_grp_link_id  = wind_lgs[ii];
                        double tmp_grp_link_val = wind_hic[ii];
                        if(tmp_node.grp_link_val_cnt[tmp_grp_link_id] == 0) tmp_node.grp_link_val_cnt[tmp_grp_link_id] = 1;
                        if(tmp_node.grp_link_val_size[tmp_grp_link_id] == 0) tmp_node.grp_link_val_size[tmp_grp_link_id] = 1;
                        // double tmp_grp_link_val_avg = tmp_node.grp_link_val_sum[tmp_grp_link_id] / tmp_node.grp_link_val_cnt[tmp_grp_link_id] / tmp_node.grp_link_val_size[tmp_grp_link_id]; 
                        double tmp_grp_link_val_avg = tmp_node.grp_link_val_sum[tmp_grp_link_id] / tmp_node.grp_link_val_cnt[tmp_grp_link_id];                        
                        //
                  ...
                  map<string, map<int, map<string, unsigned long> > >* ctg_allelic_group)
{
    ...
    */
    ...
    allelic_map_merged.insert(allelic_map.begin(), allelic_map.end());
    allelic_map_merged.insert(allelic_map_seq.begin(), allelic_map_seq.end());
    cout << "   Infox: "                             << allelic_map_seq.size() 
    ...
    //
    map<string, map<int, map<string, unsigned long> > > ctg_allelic_group_tmp; // <this_ctg, <allelic_grp, <allelic_ctg, allelic_ctg_size > > >
    //
