# Step 1:
# Here we select the potential best grouping of markers: for C, E, G, for some chrs, I did further manual selection according to diff in hap-chrs! 20221229
this_date=20221229
#
...
for(sample in c("C"))
{
  ...
  #
  min_cases <- c()
  # as chr10 does not give any phasing according to the general selection criteria, I did it with manual selection for chr10.
  for(chr in c("chr01", "chr02", "chr03", "chr04", "chr05", "chr06", "chr07", "chr08", "chr09", "chr11", "chr12"))
  #for(chr in "chr10")
  {
    # sample "O"
    ...
    for (ci in 1:length(data_chr_sorted$V1) )
    {
      ...
      for(j in 6:8)
      {
        for(k in (j+1):9)
        {
          ...
          # if(this_diff > 30000000 | data_chr_sorted[ci,16] > 18000000) # C,E, G only
    ...
    min_cases <- rbind(min_cases, min_case)
    colnames(min_case)=NULL
    print(min_case)     
  }
  write.table(x = min_cases, file=paste(binning_path, "/hic_", sample, "/bwa_align/lg_wise_contigs_read_align/final_res_",sample,"_binning_best_cases.txt", sep=""), append = F, quote = F, row.names = F, col.names = F)
}
likely_genome_assembly_size = sum(min_cases[1:12, 6:9])
